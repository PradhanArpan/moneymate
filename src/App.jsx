/* ─────────────────────────────────────────────────────────────────
   MONEYMATE v2  ·  Smart Money Tracker
   ─────────────────────────────────────────────────────────────────*/
...
import { Wallet, PiggyBank, Target, LayoutDashboard, List, Plus, Trash2, X, ChevronLeft, ChevronRight, Download, FileUp, CheckCircle2, Repeat, Edit2, ArrowLeft, TrendingUp, RefreshCw } from "lucide-react";

/* ── Tokens ──────────────────────────────────────────────────────── */
const C = {
  bg:"#F0F4F8", card:"#FFFFFF", ink:"#1A2332", muted:"#6B7A8D", border:"#E2E8F0",
  brand:"#00C896", brandDim:"#E6FBF5", income:"#00C896", expense:"#FF5A5F",
  xfer:"#5B8DEF", gold:"#FFA726", dark:"#0D1B2A",
  charts:["#FF5A5F","#FF9F43","#FFA726","#00C896","#5B8DEF","#A55EEA","#26D9A7","#FC5C7D","#6C5CE7","#00B8D9","#78909C"],
};
...
const last30Start = () => { const d=new Date(); d.setDate(d.getDate()-30); return d.toISOString().slice(0,10); };
...
function evalExpr(s) {
  ...
  const fact=()=>{if(s[i]==="("){i++;const v=expr();if(s[i]===")") i++;return v;}if(s[i]==="-"){i++;return -fact();}let j=i;while(j<s.length&&/[\d.]/.test(s[j]))j++;const v=parseFloat(s.slice(i,j));i=j;return v;};
...
const loadPdf=()=>new Promise((res,rej)=>{
  ...
  const s=document.createElement("script");s.src=`${PDF_CDN}/pdf.min.js`;
...
async function extractLines(pdf){
  ...
  for(let p=1;p<=pdf.numPages;p++){
    ...
    [...rows.keys()].sort((a,b)=>b-a).forEach(y=>{const line=rows.get(y).sort((a,b)=>a.x-b.x).map(i=>i.str).join(" ").replace(/\s+/g," ").trim();if(line)lines.push(line);});
  }
  return lines;
}
...
function PinScreen({isSetup,onSetup,onUnlock,error,setError,onForgot}){
  ...
  const push=d=>{
    ...
    const next=[...digits,String(d)];setDigits(next);
    if(next.length===4){const p=next.join("");
      setTimeout(()=>{
        if(!isSetup){onUnlock(p);setDigits([]);}
        else if(stage===1){setFirst(p);setStage(2);setDigits([]);setError("");}
        else{if(p===first)onSetup(p);else{setError("PINs don't match — try again");setStage(1);setFirst("");setDigits([]);}}
      },120);}
  };
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0D1B2A 0%,#0B3D2E 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,fontFamily:"Inter,system-ui,sans-serif"}}>
      ...
      <div style={{display:"flex",gap:18,marginBottom:36}}>
        {[0,1,2,3].map(i=><div key={i} style={{width:14,height:14,borderRadius:"50%",background:digits.length>i?C.brand:"rgba(255,255,255,0.15)",border:"2px solid rgba(255,255,255,0.2)",transition:"background 0.15s"}}/>)}
      </div>
...
export default function App(){
  ...
  const persist=async(next,pinOverride)=>{
    setData(next);
    try{localStorage.setItem("mm:data",await encryptData(next,pinOverride||pin));}
    catch(e){console.error("Save failed:",e);}
  };

  const handleSetup=async p=>{
    try{localStorage.setItem("mm:data",await encryptData(EMPTY,p));localStorage.setItem("mm:setup","1");}catch(e){console.error(e);}
    setPin(p);setData(EMPTY);setPhase("app");
  };

  const handleUnlock=async p=>{
    try{const raw=localStorage.getItem("mm:data");const d=raw?await decryptData(raw,p):EMPTY;setPin(p);setData(d);setErr("");setPhase("app");}
...
function FinanceApp({data,persist}){
  ...
  const markPaid =rec=>{
    const mk=curMo(),[y,m]=mk.split("-").map(Number),lastDay=new Date(y,m,0).getDate();
    const date=`${mk}-${String(Math.min(rec.day,lastDay)).padStart(2,"0")}`;
    persist({...data,transactions:[...data.transactions,{id:uid(),type:rec.type,amount:rec.amount,category:rec.category,accountId:rec.accountId,date,note:rec.name,source:"recurring"}],recurring:data.recurring.map(r=>r.id===rec.id?{...r,lastDone:mk}:r)});
  };
  const importTxns=(rows,accountId)=>{
    ...
    rows.forEach(r=>{const k=`${accountId}|${r.date}|${r.amount.toFixed(2)}|${r.type}`;if(ex.has(k)){skipped++;return;}ex.add(k);fresh.push({id:uid(),type:r.type,amount:r.amount,category:r.category,accountId,date:r.date,note:r.desc,source:"import"});added++;});
    persist({...data,transactions:[...data.transactions,...fresh]});return{added,skipped};
  };
  const exportCSV=()=>{
  ...
  const dIncome=todayTxns.filter(t=>t.type==="income").reduce((s,t)=>s+(+t.amount),0);
  const dExpense=todayTxns.filter(t=>t.type==="expense").reduce((s,t)=>s+(+t.amount),0);
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"Inter,system-ui,sans-serif",color:C.ink}}>
      ...
      <button onClick={()=>setModal({type:"txn"})} style={{position:"fixed",right:20,bottom:82,width:54,height:54,borderRadius:"50%",background:C.brand,border:"none",color:"#fff",cursor:"pointer",boxShadow:"0 4px 20px rgba(0,200,150,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:30}}><Plus size={22}/></button>
      ...
      {modal?.type==="editacc" &&<EditAccountModal close={()=>setModal(null)} account={modal.account} editAcc={editAcc}/>}
      {modal?.type==="goal"    &&<GoalModal     close={()=>setModal(null)} addGoal={addGoal} bankAccounts={bankAccounts}/>}
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   HOME SCREEN — Daily / Monthly toggle
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function HomeScreen({data,month,setMonth,setModal,balances,netWorth,mIncome,mExpense,dIncome,dExpense,catSpend,todayTxns,trend,pending,balances:_b}){
  const[view,setView]=useState("monthly");
  const isDaily=view==="daily";
  const inc=isDaily?dIncome:mIncome;
  const exp=isDaily?dExpense:mExpense;
  const txns=isDaily?todayTxns:null;
  const dateLabel=isDaily?new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long"}):monthLabel(month);
  const pieData=Object.entries(catSpend).sort((a,b)=>b[1]-a[1]).map(([name,value])=>({name,value}));

  return(
    <div>
      {/* Header */}
      <div style={{background:"linear-gradient(160deg,#0D1B2A 0%,#0B3D2E 100%)",padding:"52px 20px 24px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <div style={{display:"flex",background:"rgba(255,255,255,0.1)",borderRadius:20,padding:3}}>
            {["daily","monthly"].map(v=>(
              <button key={v} onClick={()=>setView(v)} style={{padding:"5px 14px",borderRadius:16,border:"none",background:view===v?"#fff":"transparent",color:view===v?C.ink:"#B0C9C0",fontSize:12,fontWeight:700,cursor:"pointer",textTransform:"capitalize",transition:"all .2s"}}>{v}</button>
            ))}
          </div>
        </div>
        ...
        {!isDaily&&(
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:14}}>
            ...
            <span style={{fontSize:13,color:"#B0C9C0",minWidth:140,textAlign:"center",fontWeight:500}}>{monthLabel(month)}</span>
            <NavBtn onClick={()=>setMonth(nextMo(month))}><ChevronRight size={16}/></NavBtn>
          </div>
        )}
        {isDaily&&<div style={{fontSize:12,color:"#6B9A8A",textAlign:"center",marginBottom:14}}>{dateLabel}</div>}
        ...
        <div style={{textAlign:"center"}}>
          ...
          <div style={{display:"flex",marginTop:16}}>
            {[["Income",inc,C.income],["Spent",exp,C.expense],["Saved",inc-exp,(inc-exp)>=0?C.income:C.expense]].map(([lbl,val,col])=>(
      ...
      {!isDaily&&(
        <SectionCard>
          {pieData.length>0?(
            <>
              <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:8}}>SPENDING BY CATEGORY — {monthLabel(month).toUpperCase()}</div>
              <div style={{position:"relative"}}>
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={pieData} dataKey="value" cx="50%" cy="50%" innerRadius={55} outerRadius={88} paddingAngle={2} onClick={s=>setModal({type:"quickadd",cat:s.name})} style={{cursor:"pointer"}}>
                ...
                <div style={{position:"absolute",inset:0,display:"grid",placeItems:"center",pointerEvents:"none"}}>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontFamily:"Georgia,serif",fontSize:18,fontWeight:700,color:C.expense}}>{inr(mExpense)}</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:10}}>
                {pieData.map(({name,value},i)=>(
                  <button key={name} onClick={()=>setModal({type:"quickadd",cat:name})} style={{display:"flex",alignItems:"center",gap:7,padding:"7px 10px",background:C.bg,borderRadius:10,border:"none",cursor:"pointer"}}>
                    <div style={{width:10,height:10,borderRadius:3,background:C.charts[i%C.charts.length],flexShrink:0}}/>
                    ...
                    <div style={{fontSize:10,color:C.muted,flexShrink:0}}>{Math.round(value/mExpense*100)}%</div>
                  </button>
                ))}
              </div>
            </>
          ):(
            <div style={{textAlign:"center",padding:"28px 0"}}><div style={{fontSize:44,marginBottom:10}}>💸</div><div style={{color:C.muted,fontSize:14}}>No expenses in {monthLabel(month)}</div><div style={{fontSize:12,color:C.muted,marginTop:4}}>Tap + to log your first entry</div></div>
          )}
        </SectionCard>
      )}
      ...
      {isDaily&&(
        <SectionCard>
          <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:10}}>TODAY'S ACTIVITY</div>
          {todayTxns.length===0?<div style={{color:C.muted,fontSize:13,textAlign:"center",padding:"16px 0"}}>No transactions today yet.</div>:
            todayTxns.slice(0,8).map(t=>{
              const isInc=t.type==="income";const isXfer=t.type==="transfer";
              return(
                <div key={t.id} style={{display:"flex",alignItems:"center",gap:10,paddingBottom:10,marginBottom:10,borderBottom:`1px solid ${C.border}`}}>
                  <span style={{fontSize:20}}>{isXfer?"↔️":CAT_EMOJI[t.category]||"📌"}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:C.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.category}</div>
                    {t.note&&<div style={{fontSize:11,color:C.muted,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.note}</div>}
                  </div>
                  <div style={{fontFamily:"Georgia,serif",fontSize:14,fontWeight:700,color:isInc?C.income:isXfer?C.xfer:C.expense}}>{isInc?"+":isXfer?"":"-"}{inr(t.amount)}</div>
                </div>
              );
            })
          }
      ...
      {pending.length>0&&(
        <div style={{margin:"12px 16px 0",background:"#FFFBEB",borderRadius:16,padding:"14px 16px",border:"1px solid #FDE68A"}}>
          <div style={{fontSize:11,fontWeight:700,color:"#92400E",letterSpacing:"0.06em",marginBottom:10}}>⏰ PLANNED THIS MONTH</div>
          {pending.map(r=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}>
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:C.ink}}>{CAT_EMOJI[r.category]||"📌"} {r.name}</div><div style={{fontSize:11,color:C.muted}}>Day {r.day} · {r.category}</div></div>
      ...
      {!isDaily&&(
        <SectionCard>
          <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:12}}>6-MONTH FLOW</div>
          ...
          <div style={{display:"flex",gap:16,justifyContent:"center",marginTop:8}}>
            {[["Income",C.income],["Expense",C.expense]].map(([l,col])=>(
              <div key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:C.muted}}><div style={{width:20,height:3,borderRadius:2,background:col}}/>{l}</div>
            ))}
          </div>
        </SectionCard>
      )}
      ...
      {data.accounts.length>0&&(
        <div style={{margin:"12px 16px 16px"}}>
          <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:10,paddingLeft:2}}>ACCOUNTS</div>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ENTRIES
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function EntriesScreen({data,month,setMonth,setModal,monthTxns,delTxn,pending,markPaid,delRec,exportCSV}){
  return(
    <div style={{padding:"52px 16px 0"}}>
      ...
      {pending.length>0&&(
        <div style={{background:"#FFFBEB",borderRadius:14,padding:"12px 14px",marginBottom:14,border:"1px solid #FDE68A"}}>
          <div style={{fontSize:11,fontWeight:700,color:"#92400E",marginBottom:8,letterSpacing:"0.06em"}}>⏰ PLANNED</div>
          {pending.map(r=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}>
      ...
      {monthTxns.length===0&&<div style={{background:C.card,borderRadius:14,padding:24,textAlign:"center",color:C.muted}}>{data.accounts.length===0?"Add an account first, then log entries.`":`No entries for ${monthLabel(month)}.`}</div>}
      <div style={{display:"grid",gap:8}}>
        {monthTxns.map(t=>{
          const acc=data.accounts.find(a=>a.id===t.accountId);const to=data.accounts.find(a=>a.id===t.toAccountId);
          ...
          return(
            <div key={t.id} style={{background:C.card,borderRadius:14,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 6px rgba(0,0,0,0.05)"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:600,color:C.ink,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isXfer?`${acc?.name||"?"} → ${to?.name||"?"}`:t.category}</div>
...
function AccountsScreen({data,setModal,balances,netWorth,delAcc,editAcc}){
  ...
  if(detail){
    const txns=data.transactions
      .filter(t=>(t.accountId===detail.id||t.toAccountId===detail.id)&&t.date>=last30Start())
    ...
    return(
      <div style={{padding:"52px 16px 0"}}>
        ...
        <div style={{background:C.dark,borderRadius:16,padding:20,marginBottom:16}}>
          ...
          <div style={{fontFamily:"Georgia,serif",fontSize:28,fontWeight:700,color:"#fff",margin:"8px 0 4px"}}>{detail.name}</div>
          <div style={{fontFamily:"Georgia,serif",fontSize:36,fontWeight:700,color:balance<0?C.expense:C.brand}}>{inr(balance)}</div>
          <div style={{display:"flex",gap:10,marginTop:16}}>
            <button onClick={()=>setModal({type:"editacc",account:detail})} style={{flex:1,background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:10,padding:"8px 0",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Edit2 size={14}/> Edit</button>
            <button onClick={()=>{if(window.confirm("Delete this account?"))delAcc(detail.id);setDetail(null);}} style={{flex:1,background:"rgba(255,90,95,0.2)",border:"1px solid rgba(255,90,95,0.3)",borderRadius:10,padding:"8px 0",color:"#FF5A5F",fontSize:13,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Trash2 size={14}/> Delete</button>
          </div>
        </div>
        <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:10}}>LAST 30 DAYS</div>
        {txns.length===0&&<div style={{background:C.card,borderRadius:14,padding:20,textAlign:"center",color:C.muted}}>No transactions in the last 30 days.</div>}
        <div style={{display:"grid",gap:8}}>
          {txns.map(t=>{
            const isInc=t.type==="income"||t.toAccountId===detail.id;
            ...
            return(
              <div key={t.id} style={{background:C.card,borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
                <div style={{width:36,height:36,borderRadius:10,display:"grid",placeItems:"center",fontSize:16,background:isXfer?"#EEF3FF":isInc?C.brandDim:"#FFEEEE",flexShrink:0}}>{isXfer?"↔️":CAT_EMOJI[t.category]||"📌"}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{isXfer?`Transfer ${t.toAccountId===detail.id?`from ${fromAcc?.name}`:`to ${toAcc?.name}`}`:t.category}</div>
                  <div style={{fontSize:11,color:C.muted}}>{new Date(t.date).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}{t.note?` · ${t.note}`:""}</div>
                </div>
                <div style={{fontFamily:"Georgia,serif",fontSize:14,fontWeight:700,color:isInc||t.toAccountId===detail.id?C.income:C.expense,flexShrink:0}}>
                  {t.toAccountId===detail.id?"+":t.type==="income"?"+":"−"}{inr(t.amount)}
                </div>
              </div>
            );
          })}
          ...
          <div key={a.id} onClick={()=>setDetail(a)} style={{background:C.card,borderRadius:16,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,0.06)",borderLeft:`4px solid ${C.charts[(i+3)%C.charts.length]}`,cursor:"pointer"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              ...
              <div style={{display:"flex",gap:6}}>
                <button onClick={e=>{e.stopPropagation();setModal({type:"editacc",account:a});}} style={iconBtn}><Edit2 size={15}/></button>
                <button onClick={e=>{e.stopPropagation();if(window.confirm("Delete account?"))delAcc(a.id);}} style={iconBtn}><Trash2 size={15}/></button>
              </div>
            </div>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   BUDGETS — with category drill-down
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function BudgetsScreen({data,setModal,catSpend,month,delBudget}){
  ...
  if(detail){
    ...
    const spent=catSpend[detail.cat]||0;const pct=Math.min(100,(spent/detail.limit)*100);const over=spent>detail.limit;
    return(
      <div style={{padding:"52px 16px 0"}}>
        <button onClick={()=>setDetail(null)} style={{display:"flex",alignItems:"center",gap:6,background:"none",border:"none",color:C.brand,fontSize:14,fontWeight:600,cursor:"pointer",marginBottom:16,padding:0}}><ArrowLeft size={18}/> Back</button>
        <div style={{background:C.card,borderRadius:16,padding:20,marginBottom:16,boxShadow:"0 2px 10px rgba(0,0,0,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}>
            ...
            <div>
              <div style={{fontSize:18,fontWeight:700,color:C.ink}}>{detail.cat}</div>
              <div style={{fontSize:13,color:C.muted}}>{monthLabel(month)}</div>
            </div>
          </div>
          ...
          <div style={{height:10,background:C.bg,borderRadius:5}}>
            <div style={{width:`${pct}%`,height:"100%",background:over?C.expense:pct>80?C.gold:C.brand,borderRadius:5,transition:"width .3s"}}/>
          </div>
        ...
        <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:10}}>TRANSACTIONS THIS MONTH</div>
        {txns.length===0&&<div style={{background:C.card,borderRadius:14,padding:20,textAlign:"center",color:C.muted}}>No expenses in this category.</div>}
        <div style={{display:"grid",gap:8}}>
          {txns.map(t=>(
            <div key={t.id} style={{background:C.card,borderRadius:12,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
              <div style={{flex:1,minWidth:0}}>
  ...
  return(
    <div style={{padding:"52px 16px 0"}}>
      ...
      {Object.keys(data.budgets).length===0&&<div style={{background:C.card,borderRadius:14,padding:28,textAlign:"center",color:C.muted}}><div style={{fontSize:40,marginBottom:10}}>🎯</div>Set monthly limits per category and track your spending.</div>}
      <div style={{display:"grid",gap:12}}>
        {Object.entries(data.budgets).map(([cat,limit])=>{
          ...
          return(
            <div key={cat} onClick={()=>setDetail({cat,limit})} style={{background:C.card,borderRadius:16,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,0.06)",cursor:"pointer"}}>
              ...
              <div style={{height:8,background:C.bg,borderRadius:4}}><div style={{width:`${pct}%`,height:"100%",background:color,borderRadius:4,transition:"width .3s"}}/></div>
              <div style={{display:"flex",justifyContent:"space-between",marginTop:8}}>
                ...
                <button onClick={e=>{e.stopPropagation();delBudget(cat);}} style={{...iconBtn,fontSize:11,color:C.muted}}>Remove</button>
              </div>
            </div>
          );
        })}
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   GOALS — bank-linked + investment types + detail view
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function GoalsScreen({data,setModal,delGoal,editGoal,balances,bankAccounts}){
  const[detail,setDetail]=useState(null);
  ...
  if(detail){
    const g=data.goals.find(x=>x.id===detail)||detail;
    if(!g){setDetail(null);return null;}
    const saved=getSaved(g);const pct=Math.min(100,(saved/g.target)*100);const done=pct>=100;
    ...
    return(
      <div style={{padding:"52px 16px 0"}}>
        ...
        <div style={{background:"linear-gradient(160deg,#0D1B2A,#0B3D2E)",borderRadius:16,padding:20,marginBottom:16,color:"#fff"}}>
          <div style={{fontSize:11,color:"#6B9A8A",textTransform:"uppercase",letterSpacing:"0.08em"}}>{g.linkType==="account"?"Bank Goal":g.linkType==="investment"?g.investmentType:"Savings Goal"}</div>
          <div style={{fontFamily:"Georgia,serif",fontSize:24,fontWeight:700,color:"#fff",margin:"6px 0"}}>{g.name}</div>
          <div style={{fontFamily:"Georgia,serif",fontSize:36,fontWeight:700,color:done?C.brand:C.gold}}>{inr(saved)}</div>
          <div style={{fontSize:12,color:"#6B9A8A"}}>of {inr(g.target)} target</div>
          <div style={{height:8,background:"rgba(255,255,255,0.1)",borderRadius:4,marginTop:16}}><div style={{width:`${pct}%`,height:"100%",background:done?C.brand:C.gold,borderRadius:4}}/></div>
          <div style={{display:"flex",justifyContent:"space-between",marginTop:8,fontSize:12,color:"#6B9A8A"}}>
        ...
        {g.linkType==="account"&&linkedAcc&&(
          <div style={{background:C.card,borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 1px 6px rgba(0,0,0,0.05)"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:8}}>LINKED ACCOUNT</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div><div style={{fontSize:14,fontWeight:700}}>{linkedAcc.name}</div><div style={{fontSize:12,color:C.muted}}>{linkedAcc.type}</div></div>
              <div style={{fontFamily:"Georgia,serif",fontSize:20,fontWeight:700,color:C.brand}}>{inr(balances[linkedAcc.id]||0)}</div>
            </div>
            <div style={{fontSize:11,color:C.muted,marginTop:8}}>Balance reflects automatically. Transfer money into this account to grow your goal.</div>
          </div>
        )}
        {g.linkType==="investment"&&(
          <div style={{background:C.card,borderRadius:14,padding:16,marginBottom:14,boxShadow:"0 1px 6px rgba(0,0,0,0.05)"}}>
            <div style={{fontSize:11,fontWeight:700,color:C.muted,letterSpacing:"0.06em",marginBottom:8}}>INVESTMENT DETAILS</div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <button onClick={()=>setModal({type:"editgoal",goal:g})} style={{marginTop:12,display:"flex",alignItems:"center",gap:6,background:C.brandDim,border:`1px solid ${C.brand}`,borderRadius:10,padding:"8px 14px",color:C.brand,fontWeight:700,fontSize:13,cursor:"pointer"}}><RefreshCw size={14}/> Update current value</button>
          </div>
        )}
        {g.linkType==="none"&&(
          <button onClick={()=>editGoal(g.id,{saved:(g.saved||0)+0})} style={{display:"none"}}>hidden</button>
        )}
        <div style={{display:"flex",gap:10}}>
          {g.linkType==="none"&&<button onClick={()=>{const a=+window.prompt("Add amount (₹):");if(a>0)editGoal(g.id,{saved:(g.saved||0)+a});}} style={{flex:1,background:C.brandDim,border:`1px solid ${C.brand}`,borderRadius:12,padding:"12px 0",color:C.brand,fontWeight:700,fontSize:14,cursor:"pointer"}}>+ Add Money</button>}
          <button onClick={()=>setModal({type:"editgoal",goal:g})} style={{flex:1,background:C.bg,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 0",color:C.muted,fontWeight:700,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Edit2 size={14}/> Edit</button>
          <button onClick={()=>{if(window.confirm("Delete this goal?")){{delGoal(g.id);setDetail(null);}}} } style={{background:"#FFF0F0",border:`1px solid #FFD0D0`,borderRadius:12,padding:"12px 16px",color:C.expense,fontWeight:700,fontSize:14,cursor:"pointer"}}><Trash2 size={16}/></button>
        </div>
      </div>
    );
  }
    ...
    <div style={{padding:"52px 16px 0"}}>
      ...
      {data.goals.length>0&&(
        <div style={{background:C.dark,borderRadius:16,padding:16,marginBottom:16}}>
          <div style={{fontSize:11,color:"#6B9A8A",textTransform:"uppercase",letterSpacing:"0.08em"}}>Total across all goals</div>
          <div style={{fontFamily:"Georgia,serif",fontSize:26,fontWeight:700,color:C.brand,marginTop:4}}>{inr(totalSaved)}</div>
          <div style={{height:6,background:"rgba(255,255,255,0.1)",borderRadius:3,marginTop:10}}><div style={{width:`${Math.min(100,(totalSaved/totalTarget)*100)}%`,height:"100%",background:C.brand,borderRadius:3}}/></div>
          <div style={{fontSize:11,color:"#6B9A8A",marginTop:6}}>{inr(totalTarget-totalSaved)} to go</div>
        </div>
      )}
      {data.goals.length===0&&<div style={{background:C.card,borderRadius:14,padding:28,textAlign:"center",color:C.muted}}><div style={{fontSize:40,marginBottom:10}}>🪙</div>Create savings goals, track investments, or link a bank account as your goal.</div>}
      <div style={{display:"grid",gap:12}}>
        {data.goals.map(g=>{
          const saved=getSaved(g);const pct=Math.min(100,(saved/g.target)*100);const done=pct>=100;
          const linkedAcc=g.linkType==="account"?data.accounts.find(a=>a.id===g.linkedAccountId):null;
          return(
            <div key={g.id} onClick={()=>setDetail(g.id)} style={{background:C.card,borderRadius:16,padding:16,boxShadow:"0 2px 10px rgba(0,0,0,0.06)",border:done?`1px solid ${C.brand}`:`1px solid ${C.border}`,cursor:"pointer"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                <div>
                  ...
                  <div style={{fontSize:11,color:C.muted,marginTop:2}}>
                    {g.linkType==="account"&&linkedAcc?`🏦 ${linkedAcc.name}`:g.linkType==="investment"?`📈 ${g.investmentType}`:"🪙 Manual savings"}
                  </div>
                </div>
                <div style={{textAlign:"right"}}>
              ...
              <div style={{fontSize:11,color:C.muted,marginTop:8}}>Tap for details →</div>
            </div>
          );
        })}
      </div>
...
function QuickAddModal({modal,close,data,addTxn}){
  const[amt,setAmt]=useState("");const[accId,setAccId]=useState(data.accounts[0]?.id||"");
  const preview=evalExpr(amt);
  return(
    <Sheet close={close}>
      <div style={{textAlign:"center",marginBottom:18}}><div style={{fontSize:40}}>{CAT_EMOJI[modal.cat]||"📌"}</div><div style={{fontSize:18,fontWeight:700,color:C.ink,marginTop:6}}>{modal.cat}</div><div style={{fontSize:12,color:C.muted}}>Quick expense</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginTop:4}}>
        ...
        <button onClick={()=>{const a=evalExpr(amt);if(!a||a<=0||!accId)return;addTxn({type:"expense",amount:a,category:modal.cat,accountId:accId,date:today(),note:""});close();}} style={{padding:"12px 0",borderRadius:12,border:"none",background:C.expense,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>Add</button>
      </div>
    </Sheet>
  );
}
...
function TxnModal({modal,close,data,addTxn,addRec,expCats,incCats,addCat}){
  const[tp,setTp]=useState("expense");const[amt,setAmt]=useState("");const[cat,setCat]=useState(expCats[0]);
  const[accId,setAccId]=useState(data.accounts[0]?.id||"");const[toId,setToId]=useState(data.accounts[1]?.id||data.accounts[0]?.id||"");
  const[date,setDate]=useState(today());const[note,setNote]=useState("");const[repeat,setRepeat]=useState(false);
  const[newCat,setNewCat]=useState("");const[showNew,setShowNew]=useState(false);
  const cats=tp==="income"?incCats:expCats;const preview=evalExpr(amt);
  const submit=()=>{const a=evalExpr(amt);if(!a||a<=0||!accId)return;
    ...
    <Sheet close={close} title="New Entry">
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {["expense","income","transfer"].map(t=>(
          <button key={t} onClick={()=>{setTp(t);setCat(t==="income"?incCats[0]:expCats[0]);}} style={{flex:1,padding:"9px 0",borderRadius:10,border:`1px solid ${tp===t?C.brand:C.border}`,background:tp===t?C.brandDim:"#fff",color:tp===t?C.brand:C.muted,fontSize:12,fontWeight:700,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>
        ))}
      </div>
      <label style={lbl}>Amount (₹)</label>
      ...
      {tp!=="transfer"&&<>
        ...
        {showNew&&<div style={{display:"flex",gap:6,marginTop:-8,marginBottom:12}}>
      ...
      <label style={lbl}>{tp==="transfer"?"From":"Account"}</label>
      <select style={field} value={accId} onChange={e=>setAccId(e.target.value)}>{data.accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select>
      {tp==="transfer"&&<><label style={lbl}>To account</label><select style={field} value={toId} onChange={e=>setToId(e.target.value)}>{data.accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></>}
      <label style={lbl}>Date</label><input style={field} type="date" value={date} onChange={e=>setDate(e.target.value)}/>
      <label style={lbl}>Note (optional)</label><input style={field} value={note} onChange={e=>setNote(e.target.value)} placeholder="e.g. groceries at DMart"/>
      {tp!=="transfer"&&<label style={{display:"flex",alignItems:"center",gap:8,fontSize:13,color:C.ink,marginBottom:14,cursor:"pointer"}}><input type="checkbox" checked={repeat} onChange={e=>setRepeat(e.target.checked)} style={{width:16,height:16,accentColor:C.brand}}/>Repeat every month</label>}
...
function AccountModal({close,addAcc}){
  const[f,setF]=useState({name:"",type:ACCOUNT_TYPES[0],opening:"",hint:""});const s=k=>e=>setF({...f,[k]:e.target.value});
...
function EditAccountModal({close,account,editAcc}){
  ...
  return(<Sheet close={close} title="Edit Account">
    <label style={lbl}>Account name</label><input style={field} value={f.name} onChange={s("name")}/>
    <label style={lbl}>Opening / adjusted balance (₹)</label><input style={field} type="number" value={f.opening} onChange={s("opening")}/>
    <p style={{fontSize:12,color:C.muted,marginTop:-8}}>This is the starting balance. Transactions are added on top of this.</p>
    <label style={lbl}>Last 4 digits</label><input style={field} inputMode="numeric" maxLength={4} value={f.hint} onChange={s("hint")}/>
    <button onClick={()=>{if(!f.name)return;editAcc(account.id,{name:f.name,opening:+f.opening||0,hint:f.hint.trim()});close();}} style={saveBtn}>Save Changes</button>
  </Sheet>);
}

function GoalModal({close,addGoal,bankAccounts}){
  const[name,setName]=useState("");const[target,setTarget]=useState("");
  const[linkType,setLinkType]=useState("none");const[linkedAccountId,setLinkedAccountId]=useState(bankAccounts[0]?.id||"");
  const[investmentType,setInvestmentType]=useState(INVEST_TYPES[0]);const[investmentValue,setInvestmentValue]=useState("");
  const[saved,setSaved]=useState("");
  const submit=()=>{
    if(!name||!target)return;
    const g={name,target:+target,linkType};
    if(linkType==="none")g.saved=+saved||0;
    if(linkType==="account")g.linkedAccountId=linkedAccountId;
    if(linkType==="investment"){g.investmentType=investmentType;g.investmentValue=+investmentValue||0;g.investmentValueDate=today();}
    addGoal(g);close();
  };
  return(<Sheet close={close} title="New Goal">
    <label style={lbl}>Goal name</label><input style={field} value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Emergency Fund"/>
    <label style={lbl}>Target amount (₹)</label><input style={field} type="number" value={target} onChange={e=>setTarget(e.target.value)} placeholder="100000"/>
    <label style={lbl}>Link type</label>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:14}}>
      {[["none","🪙 Manual"],["account","🏦 Bank"],["investment","📈 Investment"]].map(([v,l])=>(
        <button key={v} onClick={()=>setLinkType(v)} style={{padding:"10px 4px",borderRadius:10,border:`1px solid ${linkType===v?C.brand:C.border}`,background:linkType===v?C.brandDim:"#fff",color:linkType===v?C.brand:C.muted,fontSize:11,fontWeight:700,cursor:"pointer",textAlign:"center"}}>{l}</button>
      ))}
    </div>
    {linkType==="none"&&<><label style={lbl}>Already saved (₹)</label><input style={field} type="number" value={saved} onChange={e=>setSaved(e.target.value)} placeholder="0"/></>}
    {linkType==="account"&&(bankAccounts.length>0?(
      <><label style={lbl}>Link to bank account</label><select style={field} value={linkedAccountId} onChange={e=>setLinkedAccountId(e.target.value)}>{bankAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><p style={{fontSize:12,color:C.muted,marginTop:-8}}>Goal progress = current account balance.</p></>
    ):<p style={{fontSize:13,color:C.expense}}>No Bank accounts found. Add a Bank account first.</p>)}
    {linkType==="investment"&&<>
      <label style={lbl}>Investment type</label><select style={field} value={investmentType} onChange={e=>setInvestmentType(e.target.value)}>{INVEST_TYPES.map(t=><option key={t}>{t}</option>)}</select>
      <label style={lbl}>Current value (₹)</label><input style={field} type="number" value={investmentValue} onChange={e=>setInvestmentValue(e.target.value)} placeholder="0"/>
    </>}
    <button onClick={submit} style={saveBtn}>Save Goal</button>
  </Sheet>);
}

function EditGoalModal({close,goal,editGoal,bankAccounts}){
  const[investmentValue,setInvestmentValue]=useState(goal.investmentValue||"");
  const[name,setName]=useState(goal.name);const[target,setTarget]=useState(goal.target);
  if(goal.linkType==="investment"){
    return(<Sheet close={close} title="Update Value">
      <div style={{textAlign:"center",marginBottom:18}}><div style={{fontSize:11,color:C.muted,textTransform:"uppercase"}}>Current value</div><div style={{fontFamily:"Georgia,serif",fontSize:28,fontWeight:700,color:C.gold}}>{inr(goal.investmentValue||0)}</div><div style={{fontSize:12,color:C.muted}}>{goal.investmentType}</div></div>
      <label style={lbl}>New current value (₹)</label>
      <input autoFocus style={field} type="number" value={investmentValue} onChange={e=>setInvestmentValue(e.target.value)} placeholder="Enter latest value"/>
      <button onClick={()=>{if(!investmentValue)return;editGoal(goal.id,{investmentValue:+investmentValue,investmentValueDate:today()});close();}} style={saveBtn}>Update Value</button>
    </Sheet>);
  }
  return(<Sheet close={close} title="Edit Goal">
    <label style={lbl}>Goal name</label><input style={field} value={name} onChange={e=>setName(e.target.value)}/>
    <label style={lbl}>Target (₹)</label><input style={field} type="number" value={target} onChange={e=>setTarget(e.target.value)}/>
    <button onClick={()=>{if(!name||!target)return;editGoal(goal.id,{name,target:+target});close();}} style={saveBtn}>Save Changes</button>
  </Sheet>);
}

function BudgetModal({close,setBudget,expCats}){
  const[cat,setCat]=useState(expCats[0]);const[amt,setAmt]=useState("");
...
function ImportModal({close,data,importTxns,expCats,incCats}){
  const[step,setStep]=useState("pick");const[file,setFile]=useState(null);const[pwd,setPwd]=useState("");
  const[needPwd,setNeedPwd]=useState(false);const[accId,setAccId]=useState(data.accounts[0]?.id||"");
  const[rows,setRows]=useState([]);const[err,setErr]=useState("");const[result,setResult]=useState(null);

  const parse=async()=>{
    if(!file)return;setErr("");setStep("parsing");
    try{
      const lib=await loadPdf();const buf=await file.arrayBuffer();let pdf;
      try{pdf=await lib.getDocument({data:buf,password:pwd||undefined}).promise;}
      catch(e){if(e&&(e.name==="PasswordException"||/password/i.test(e.message||""))){setNeedPwd(true);setErr(pwd?"Wrong password — try again.":"This PDF is password-protected. Enter the password (usually DOB or mobile number).");setStep("pick");return;}throw e;}
  ...
  return(<Sheet close={close} title="Import Statement">
    {step==="pick"&&<>
      <p style={{fontSize:13,color:C.muted,marginTop:0}}>Pick a bank e-statement PDF. Everything is parsed on your device — nothing is uploaded.</p>
      <label style={lbl}>PDF file</label><input type="file" accept="application/pdf" style={{...field,padding:8}} onChange={e=>{setFile(e.target.files[0]||null);setErr("");}}/>
      ...
      <select style={field} value={accId} onChange={e=>setAccId(e.target.value)}>{data.accounts.map(a=><option key={a.id} value={a.id}>{a.name}{a.hint?` ··${a.hint}`:""}</option>)}</select>
    ...
    {step==="review"&&<>
      <p style={{fontSize:13,color:C.muted,marginTop:0}}>Found <b style={{color:C.ink}}>{rows.length}</b> transactions. Review and untick anything incorrect.</p>
      <label style={lbl}>Account</label><select style={field} value={accId} onChange={e=>setAccId(e.target.value)}>{data.accounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select>
      ...
      <button onClick={doImport} disabled={!included} style={{...saveBtn,marginTop:12,background:included?C.brand:"#ccc",cursor:included?"pointer":"default",position:"sticky",bottom:0}}>Import {included} transaction{included===1?"":"s"}</button>
    </>}
    {step==="done"&&result&&<div style={{textAlign:"center",padding:"28px 0"}}>
...
function Sheet({children,close,title}){
  return(<div onClick={close} style={{position:"fixed",inset:0,background:"rgba(10,20,30,0.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:50}}>
...
function SectionCard({children}){return<div style={{margin:"16px 16px 0",background:C.card,borderRadius:16,padding:16,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>{children}</div>;}
function NavBtn({children,onClick}){return<button onClick={onClick} style={{background:"rgba(255,255,255,0.1)",border:"none",color:"#fff",width:30,height:30,borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{children}</button>;}
function PrimaryBtn({children,onClick}){return<button onClick={onClick} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:12,border:"none",background:C.brand,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer"}}>{children}</button>;}
